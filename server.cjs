...
function homePage() {
  ...
  html = html
    ...
    .replaceAll('{{heroSecondaryCta}}', hero.secondary_cta_label || 'Kite Login')
    .replaceAll('{{heroSecondaryUrl}}', hero.secondary_cta_url || 'https://our-kites.kiteagency-eg.com/')
  ...
  return layout(html, {
    ...
    extraHead: '<link rel="stylesheet" href="/assets/css/kite-intro.css?v=kite-sky-16">',
...
function publicRoute(urlPath) {
  ...
  if (urlPath === '/services') {
    return {
      html: layout(
        pubRender.servicesHorizon(pub.services, pub.projects),
