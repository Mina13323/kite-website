...
function servicesHorizon(services, projects) {
  const tabs = [
    ...
    },
    {
      ...
      tagline: 'Ideas that move people — and business',
      desc: 'We turn strategy into social media, content, campaigns, and paid media designed to earn attention and drive action.',
    ...
    },
    {
      ...
      title: 'Media<br>Production',
      tagline: 'Bring the idea to life',
      desc: 'From photography and video to motion and creative production, we turn concepts into stories people can see, feel, and remember.',
    ...
    },
    {
      ...
      tagline: 'Built around the brand. Made to work',
      desc: 'We design and develop websites and digital experiences that look like your brand, work smoothly, and move the business forward.',
  ...
  const vectorPath = 'M3384.17 2321.93L1881.77 2263.67C1769.98 2259.35 1658.02 2259.35 1546.23 2263.67L43.8264 2321.93C18.9573 2322.9 -1.35528 2303.12 0.0707804 2279.35L53.4262 1387.67C62.4695 1236.67 62.4695 1085.26 53.4262 934.26L0.105562 42.6122C-1.32049 18.8429 18.9921 -0.9315 43.8612 0.0339186L1546.26 58.292C1658.05 62.6197 1770.02 62.6197 1881.81 58.292L3384.17 0.0339186C3409.04 -0.9315 3429.36 18.8429 3427.93 42.6122L3374.57 934.293C3365.53 1085.3 3365.53 1236.7 3374.57 1387.71L3427.93 2279.39C3429.36 2303.16 3409.04 2322.93 3384.17 2321.97V2321.93Z';

  const navHtml = tabs.map((tab, i) => {
    const isActive = i === 0;
    return `<button type="button" class="svc-tab-btn${isActive ? ' is-active' : ''}" data-tab="${esc(tab.id)}" role="tab" aria-selected="${isActive ? 'true' : 'false'}" aria-controls="svc-pane-${esc(tab.id)}">${esc(tab.nav)}</button>`;
  }).join('');

  const panesHtml = tabs.map((tab, i) => {
    const isActive = i === 0;
    return `<div class="svc-pane${isActive ? ' is-active' : ''}" id="svc-pane-${esc(tab.id)}" data-pane="${esc(tab.id)}" role="tabpanel">
      <div class="svc-pane-inner">
        <div class="svc-copy-col">
          <h3 class="svc-pane-title">${tab.title}</h3>
          <h4 class="svc-pane-tagline">${esc(tab.tagline)}</h4>
          <p class="svc-pane-desc">${esc(tab.desc)}</p>
          <a href="${esc(tab.link)}" class="svc-explore-btn">
            <span>${esc(tab.btn)}</span>
            <svg class="svc-arrow-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="svc-visual-col">
          <div class="svc-visual-frame">
            ...
            <div class="svc-visual-content">
              <img src="${esc(tab.image)}" alt="${esc(tab.nav)}" class="svc-visual-img">
            </div>
          </div>
        </div>
      </div>
...
function homeSections(pub) {
  ...
  const latest = (featured_projects.length ? featured_projects : projects).slice(0, 3);
  const vectorItem = `<div class="latest-vector" aria-hidden="true"><img src="/assets/kite/Vector.svg" alt=""></div>`;
  const gridCards = [];
  for (let i = 0; i < 3; i++) {
    const p = latest[i];
    if (p) {
      const img = p.cover_image || p.hero_image;
      gridCards.push(`<a class="latest-card ${img ? '' : 'has-empty'}" href="/project/${esc(p.slug)}">
        ${img ? `<img src="${esc(img)}" alt="${esc(p.title)}">` : '<div class="empty-visual"></div>'}
        <figcaption><strong>${esc(p.title)}</strong></figcaption>
      </a>`);
    } else {
      gridCards.push(`<a class="latest-card has-empty" href="/portfolio">
        <div class="empty-visual"></div>
      </a>`);
    }
    gridCards.push(vectorItem);
  }
  const latestGridHtml = gridCards.join('');
